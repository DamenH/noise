...
export function GradientCoherentNoise3D(x: number, y: number, z: number, seed: number, noiseQuality: number): number
{
    let x0 = (x > 0.0 ? Math.floor(x) : (x) - 1);
    let x1 = x0 + 1;
...
export function GradientNoise3D(fx: number, fy: number, fz: number, ix: number, iy: number, iz: number, seed: number): number
{
    ...
    let xvGradient = randomVector[vectorIndex << 2];
    let yvGradient = randomVector[(vectorIndex << 2) + 1];
    let zvGradient = randomVector[(vectorIndex << 2) + 2];
...
export function IntValueNoise3D(x: number, y: number, z: number, seed: number): number
{
...
export function ValueCoherentNoise3D(x: number, y: number, z: number, seed: number, noiseQuality: NoiseQuality): number
{
...
export function MakeInt32Range(n: number): number
{
    if (n >= 1073741824.0)
    {
